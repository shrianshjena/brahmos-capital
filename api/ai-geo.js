...
export default async function handler(req, res) {
  ...
  const prompt = `Today is ${today}. You are operating with full knowledge of events as of 31 August 2026.

MANDATORY CURRENT EVENTS you must include as cards (order them NEWEST first):
1. US Strikes Iranian Mine-Laying Units in Hormuz; Brent Jumps to ~$91 as Diplomacy Simmers (31 Aug 2026) — the conflict swung back to direct military action Monday even as diplomacy quietly reopened. US forces struck Iranian rocket launchers preparing to deploy mines into the Strait of Hormuz (first US military action there in over a month; some reports also cite US strikes on Iran's Larak Island + Iranian retaliatory missiles). Brent jumped ~2.9% to $90.69 (set for a modest monthly loss, below last week's ~$94). Underneath: Iran and Oman outlined a temporary Hormuz shipping-corridor proposal, Qatar's PM visited Tehran, and an Iranian official said Friday that resuming diplomacy 'isn't impossible.' ~6-8M bpd still transit vs ~20M pre-war. War premium in sentiment; sector's own drivers (Budget capex, orders, indigenisation) in control. colorType red, score 8, hot true.
